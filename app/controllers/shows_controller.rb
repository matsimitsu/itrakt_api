class ShowsController < ApplicationController
  ...
  def seasons
    ...
    result = Rails.cache.fetch("seasons_#{tvdb_id}_#{@username}", :expires_in => 1.days) do
  ...
  def season
    ...
    result = Rails.cache.fetch("season_#{tvdb_id}_#{season_number}_#{@username}", :expires_in => 1.days) do
  ...
  def seasons_with_episodes
    tvdb_id = params[:tvdb_id]
    result = Rails.cache.fetch("season_with_episodes_#{tvdb_id}_#{@username}", :expires_in => 1.day) do
